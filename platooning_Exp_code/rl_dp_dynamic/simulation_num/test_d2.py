...
d2_list = [10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0, 110.0, 120.0]

#define the three running mode
#runner_mode_list = ['Basic', 'CDC', 'DP']
runner_mode_list = ['CDC']
	...
	for runner_mode in runner_mode_list:
		...
		# the CDC policy
			...
			for j in range(len(depart_time_list)-1):
				...
				threshold = rc.r_calculaton(estimated_arrval_rate, w_1, d2, 24.0)
...
plt.plot(d2_list, total_cost_basic, label='Basic')
plt.plot(d2_list, total_cost_cdc, label='CDC')
plt.plot(d2_list, total_cost_dp, label='DP')

plt.xlabel('Crusing Distance [Km]', fontsize=25)
plt.ylabel('Average Cost per 10 Km [$]', fontsize=25)
