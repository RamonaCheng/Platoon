...
@Description: 
@LastEditTime: 2023-04-14 22:33:46
'''
import gym
import random
import numpy as np
...
class JuctionAgent:
    def __init__(self, observation_space, action_space):
        ...
        self.Qmatrix = [[354.1666666666667, 291.6666666666667], [354.1666666666667, 354.1666666666667]]           # end2_link17, end2_link18, end3_link17, end3_link17
        ...
        self.InLinks = ['link2']
        self.OutLinks = ['link17', 'link18']
        self.InNodes = [1]
        self.OutNodes = [6, 8]
        #self.OutLength = [7.0, 14.0]         # kilometers
        ...
        self.action = 18                                # 17 represents link17, 18 represents link18
        ...
        self.initial_value = [3.0, 15.0, -15.0]
        self.threshold = self.initial_value[1]
        self.C = self.initial_value[2]
        self.LEARNING_RATE = LEARNING_RATE
        self._lambda = 1
        self._switch = 1 # 切换道路是否通畅
        self.dqn_file = open('./dqn12.csv', 'w', encoding='utf-8')
    ...
    def reset(self, learning_rate, _lambda):
        self.temp_memory = deque(maxlen=MAX_MEMORY)
        self.memory = deque(maxlen=MAX_MEMORY)
        self.Qmatrix = [[354.1666666666667, 291.6666666666667], [354.1666666666667, 354.1666666666667]]
        self.lead_vehicle = None
        self.temp_vehicle = None
        self.lead_time = 0.0
        self.temp_time = 0.0
    ...
    def get_state(self, junction_vehicle, junction_time):
        if self.vehicle_count == 0:
            ...
            if temp_des == 2:
                ...
                traci.vehicle.setVia(junction_vehicle, ['link18'])
                traci.vehicle.rerouteEffort(junction_vehicle)
            
                # change the current vehicle as the leading vehicle
                self.lead_vehicle_link = 18
                self.lead_path = dis.return_path(12, temp_des, dis.return_initial_Qmatrix_Table())
            elif temp_des == 3:
                # select the shorest path
                traci.vehicle.setVia(junction_vehicle, ['link17'])
                traci.vehicle.rerouteEffort(junction_vehicle)
            
                # change the current vehicle as the leading vehicle
                self.lead_vehicle_link = 17
                self.lead_path = dis.return_path(12, temp_des, dis.return_initial_Qmatrix_Table())
            ...
            if self.temp_vehicle in self.vehicle_container:
                ...
                #print('junction 12 state: ', self.state)
    ...
    def take_action(self, state, Qmatrix_table):
        ...
        if temp_des == 2:
            if p < exploration_rate:
                self.action = random.choice([17,18])
            else:
        ...
        if d2_distance == 0 or sk > int(d1/24)-2:
            ...
            final_value = threshold_func.theta_c(self.arrival_rate, [3.0, 15.0, -15.0], w_1/3600.0, gamma, original_equiva_distance, 24.0, w_2)
            if abs(final_value[2]) > 50 or (final_value[1] < 0 and final_value[2] > 0):
                final_value = [3.0, 15.0, -15.0]
            ...
            #print('junction 12 final_value: ', final_value)
            if abs(final_value[2]) > 50 or (final_value[1] < 0 and final_value[2] > 0):
                final_value = [3.0, 15.0, -15.0]
        ...
        self.dqn_file.write(f'{traci.simulation.getTime()}, {self.C}, {self.threshold}, {self.arrival_rate}\n')
        self.dqn_file.flush()
        #self.action_platoon = 0
        #self.C = collision_time_delay
        
        #print('junction 12 action: ', self.action)
    ...
    def coordinate(self, detected_fuel, detected_time):
        ...
        if self.action == 17:   
            traci.vehicle.setVia(self.temp_vehicle, ['link17'])
        elif self.action == 18:
            traci.vehicle.setVia(self.temp_vehicle, ['link18'])
    ...
    def return_time_matrix(self):
        if (traci.simulation.getTime() > 1000) and (traci.simulation.getTime() < 2000):
            self.Qmatrix[0][1] = 9999
        elif (traci.simulation.getTime() > 2000) and self._switch == 1:
            self.Qmatrix[0][1] = 291.6666666666667
            self._switch += 1 # 只重置一次
        return self.Qmatrix
