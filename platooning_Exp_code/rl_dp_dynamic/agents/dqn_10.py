...
@Description: 
@LastEditTime: 2023-04-14 22:33:23
'''
import gym
import random
import numpy as np
...
class JuctionAgent:
    def __init__(self, observation_space, action_space):
        ...
        self.Qmatrix = [187.5, 187.5]      # end2_link14, end3_link14
        ...
        self.InLinks = ['link8', 'link12']
        self.OutLinks = ['link14']
        self.InNodes = [6, 9]
        self.OutNodes = [11]
        #self.OutLength = [6.0]         # kilometers
        ...
        self.action = 14                                # 14 represents link14
        ...
        self.initial_value = [6.0, 11.0, -11.0]
        self.threshold = self.initial_value[1]
        self.C = self.initial_value[2]
        self.LEARNING_RATE = LEARNING_RATE
        self._lambda = 1
        self.dqn_file = open('./dqn10.csv', 'w', encoding='utf-8')
    ...
    def get_state(self, junction_vehicle, junction_time):
        if self.vehicle_count == 0:
            ...
            self.lead_path = dis.return_path(10, temp_des, dis.return_initial_Qmatrix_Table())
            ...
            if self.temp_vehicle in self.vehicle_container:
                ...
                #print('junction 10 state: ', self.state)
    ...
    def take_action(self, state, Qmatrix_table):
        ...
        #print(current_node, lead_des, temp_des, lead_action, self.action, Qmatrix_table, self.lead_path, temp_path)
        ...
        if d2_distance == 0 or sk > int(d1/24)-2:
            ...
            if abs(final_value[2]) > 50 or (final_value[1] < 0 and final_value[2] > 0):
                final_value = [6.0, 11.0, -11.0]
            self.C = final_value[2]

            self.initial_value = final_value
        else:
            # using the threshold and C strategy
            #print(self.arrival_rate, self.initial_value, w_1/3600.0, gamma, d2_distance, 24.0, w_2)
        ...
        #print('junction 10 action: ', self.action)
    ...
    def update_onestep(self):
        ...
        if temp_des == 2:
            min_q = self.Qmatrix[0]
        elif temp_des == 3:
            min_q = self.Qmatrix[1]
    ...
    def experience_replay(self):
        if len(self.memory) < BATCH_SIZE:
            ...
            for state, action, min_q, cost in minibatch:
                ...
                if temp_des == 2:
                    self.Qmatrix[0] = self.Qmatrix[0] + self.LEARNING_RATE * (Q - self.Qmatrix[0])
                elif temp_des == 3:
                    self.Qmatrix[1] = self.Qmatrix[1] + self.LEARNING_RATE * (Q - self.Qmatrix[1])
