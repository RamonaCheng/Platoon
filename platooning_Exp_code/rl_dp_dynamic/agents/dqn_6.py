...
@Description: 
@LastEditTime: 2023-04-14 22:31:58
'''
import gym
import random
import numpy as np
...
class JuctionAgent:
    def __init__(self, observation_space, action_space):
        ...
        self.Qmatrix = [[270.8333333333333, 270.8333333333333], [270.8333333333333, 270.8333333333333]]      # end2_link7, end2_link8, end3_link7, end3_link8
        ...
        #self.initial_value = [1.0, 20.0, -25.0]         # initial value for the threshold and C value
        self.initial_value = [2.0, 10.0, -10.0]
        self.threshold = self.initial_value[1]
        self.C = self.initial_value[2]
        self.LEARNING_RATE = LEARNING_RATE
        self._lambda = 1 # lambda*exp_travel_time + (1-lambda)*this_travel_time
        self.dqn_file = open('./dqn6.csv', 'w', encoding='utf-8')
    ...
    def reset(self, learning_rate, _lambda=1):
        self.temp_memory = deque(maxlen=MAX_MEMORY)
        self.memory = deque(maxlen=MAX_MEMORY)
        self.Qmatrix = [[270.8333333333333, 270.8333333333333], [270.8333333333333, 270.8333333333333]]
        self.lead_vehicle = None
        self.temp_vehicle = None
        self.lead_time = 0.0
        self.temp_time = 0.0
        ...
        #self.initial_value = [1.0, 20.0, -25.0]
        self.initial_value = [2.0, 10.0, -10.0]         
        self.threshold = self.initial_value[1]
        self.C = self.initial_value[2]
        self.LEARNING_RATE = learning_rate
    ...
    def get_state(self, junction_vehicle, junction_time):
        if self.vehicle_count == 0:
            ...
            if temp_des == 2:
                ...
                self.lead_path = dis.return_path(6, temp_des, dis.return_initial_Qmatrix_Table())
            elif temp_des == 3:
                # select the shorest path
                traci.vehicle.setVia(junction_vehicle, ['link7'])
                traci.vehicle.rerouteEffort(junction_vehicle)
    ...
    def take_action(self, state, Qmatrix_table):
        ...
        if self.action == 7:
            temp_path = [7] + dis.return_path(7, temp_des, Qmatrix_table)
        elif self.action == 8:
        ...
        if d2_distance == 0 or sk > int(d1/24)-2:
            ...
            final_value = threshold_func.theta_c(self.arrival_rate, [2.0, 10.0, -10.0], w_1/3600.0, gamma, original_equiva_distance, 24.0, w_2)
            if abs(final_value[2]) > 50 or (final_value[1] < 0 and final_value[2] > 0):
                final_value = [2.0, 10.0, -10.0]
