...
class JuctionAgent:
    def __init__(self, observation_space, action_space):
        ...
        # [s / tarvel time] end2_link1, end2_link2, end3_link1, end3_link2
        ...
        # 1 represents link1, 2 represents link2
        self.action = 1
        # 0 represents travel alone, 1 represents platooning
    ...
    def get_state(self, junction_vehicle, junction_time):
        if self.vehicle_count == 0:
            ...
            if temp_des == 2:
                ...
                self.lead_path = dis.return_path(
                    1, temp_des, dis.return_initial_Qmatrix_Table())
            elif temp_des == 3:
                # select the shorest path
                traci.vehicle.setVia(junction_vehicle, ['link1'])
                traci.vehicle.rerouteEffort(junction_vehicle)
            ...
            if self.temp_vehicle in self.vehicle_container:
                ...
                for i_pop_list in range(len(self.time_interval_list)):
                    estimated_arrval_rate += self.time_interval_list[len(
                        self.time_interval_list) - 1 - i_pop_list] * (gamma_estimator ** gamma_index)
                    gamma_index += 1

                estimated_arrval_rate = estimated_arrval_rate * \
                    (1.0 - gamma_estimator)
                estimated_arrval_rate = 1.0 / \
                    estimated_arrval_rate if estimated_arrval_rate != 0 else 10
                self.arrival_rate = estimated_arrval_rate

                # get the vehicle state
                self.lead_vehicle_des, self.temp_vehicle_des = self.get_veh_des(
                    self.lead_vehicle, self.temp_vehicle)
                self.state = [self.sk, self.lead_vehicle_des,
                              self.temp_vehicle_des, self.lead_vehicle_link]
    ...
    def take_action(self, state, Qmatrix_table):
        ...
        d2_distance = dis.d2_distance(current_node, lead_des, temp_des,
                                      lead_action, self.action, Qmatrix_table, self.lead_path, temp_path)
        ...
        if d2_distance == 0 or sk > (d1/24)-3:
            ...
            threshold = rc.r_calculaton(
                self.arrival_rate, w_1/3600.0, d2_distance)
    ...
    def experience_replay(self):
        if len(self.memory) < BATCH_SIZE:
            ...
            for state, action, min_q, cost in minibatch:
                ...
                if temp_des == 2:
                    if action == 1:
                        self.Qmatrix[0][0] = self.Qmatrix[0][0] + \
                            self.LEARNING_RATE * (Q - self.Qmatrix[0][0])
                    elif action == 2:
                        self.Qmatrix[0][1] = self.Qmatrix[0][1] + \
                            self.LEARNING_RATE * (Q - self.Qmatrix[0][1])
                elif temp_des == 3:
                    if action == 1:
                        self.Qmatrix[1][0] = self.Qmatrix[1][0] + \
                            self.LEARNING_RATE * (Q - self.Qmatrix[1][0])
                    elif action == 2:
                        self.Qmatrix[1][1] = self.Qmatrix[1][1] + \
                            self.LEARNING_RATE * (Q - self.Qmatrix[1][1])
    ...
    def coordinate(self, detected_fuel, detected_time):
        ...
        # print('set_follower_speed: ', set_follower_speed)
    ...
    def add_to_memory(self, current_vehicle_id, min_q, departure_link_fuel, departure_link_time):
        ...
        for item in list(self.temp_memory):
            if current_vehicle_id == item[0]:
                ...
                pos_speed = [
                    (traci.vehicle.getLanePosition(_veh_id),
                     traci.vehicle.getSpeed(_veh_id))
                    for _veh_id in veh_ids
                ]
                v_speeds_list = [i[1] for i in filter(
                    self._f, pos_speed)]  # get vehicle speed
                last_time_speed = 24 if len(
                    v_speeds_list) == 0 else np.max(v_speeds_list)
