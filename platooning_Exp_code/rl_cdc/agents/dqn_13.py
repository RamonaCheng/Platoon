...
@Description: 
@LastEditTime: 2023-04-19 20:40:01
'''
import gym
import random
import numpy as np
...
class JuctionAgent:
    def __init__(self, observation_space, action_space):
        ...
        # self.Qmatrix = [500.0]      # end2_link11
        self.Qmatrix = [125.0]      # end2_link11
    ...
    def take_action(self, state, Qmatrix_table):
        ...
        if d2_distance == 0 or sk > (d1/24)-3:
            self.action_platoon = 0 # 不进行编队
        else:
            # Calculate the threshold
            threshold = rc.r_calculaton(self.arrival_rate, w_1/3600.0, d2_distance)
            # time_interval 是指和 lead vehicle 相差的时间
            # threshold 是指计算出来的追赶上的时间
            if self.time_interval > threshold:
                self.action_platoon = 0 # 不进行编队
            else:
                self.action_platoon = 1 # 进行编队
