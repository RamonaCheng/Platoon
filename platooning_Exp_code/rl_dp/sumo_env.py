...
@Description: 
@LastEditTime: 2024-01-06 01:06:11
'''
import os
import sys 
            ...
            for item in traci.edge.getLastStepVehicleIDs(self.incLanes[i]):
                if distance(self.nodePos,traci.vehicle.getPosition(item))/distance(self.nodePos,self.incLanesOBJ[i].getFromNode().getCoord()) > (d1+100)/(self.incLanesOBJ[i].getLength()):
                    ...
                    if self.incLanes[i] not in ['start1', 'start2'] and vehicle_type == 'connected_pLeader':
                        ...
                        # 将 density 存储下来，绘制一个分布
                        speed_limit = max(24.0 * (1.0 - density / self.jam_density), 4.0)
                        # print('density', speed_limit, density, density/self.jam_density, traci.edge.getLastStepVehicleNumber(self.incLanes[i]), following_vehicle_num, link_length)
...
class network:
    def __init__(self, path, ui, sumocfgPath, routePath, steptime=500, jam_density=0.0095):
        ...
        self.cost_cav = 0.0
        self.vehicle_fuel_cost = 0.0
        self.vehciel_time_cost = 0.0
    ...
    def observe(self):
        ...
        # calculate the average platoon size
        #platoon_num = 0
        #total_num = 0
        for vehicle in traci.vehicle.getIDList():
            ...
            if (vehicle_item_type == 'connected_pFollower' or vehicle_item_type == 'connected_pCatchup' or vehicle_item_type == 'connected_pCatchupFollower'):
                ...
                traci.vehicle.setColor(vehicle,(0,0,205))   # Blue
                #total_num += 1
        ...
        #if platoon_num > 0:
        #    print('Average platoon size: ', total_num, platoon_num, float(total_num)/float(platoon_num))
        ...
        for left_vehicle in traci.simulation.getArrivedIDList():
            if left_vehicle in self.non_cav_list:
                ...
                self.vehciel_time_cost += self.vehicle_time[left_vehicle]/3600.0 * w_1
                self.vehicle_fuel_cost += self.vehicle_fuel[left_vehicle]/1000.0 * w_2
    ...
    def return_cost_cav(self):
        return f'Total Cost: {self.cost_cav} | Fuel Cost: {self.vehicle_fuel_cost} | Time Cost: {self.vehciel_time_cost}'
