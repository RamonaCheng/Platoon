...
class JuctionAgent:
    ...
    def get_state(self, junction_vehicle, junction_time):
        if self.vehicle_count == 0:
            ...
            if temp_des == 2:
                ...
                self.lead_path = traci.simulation.findRoute('link1', 'link11', 'connected').edges
            elif temp_des == 3:
                # select the shorest path
                traci.vehicle.setVia(junction_vehicle, ['link1'])
                traci.vehicle.rerouteEffort(junction_vehicle)
                ...
                # change the current vehicle as the leading vehicle
                self.lead_vehicle_link = 1
                self.lead_path = traci.simulation.findRoute('link1', 'link16', 'connected').edges
    ...
    def take_action(self, state, junction_vehicle):
        ...
        # traci.simulation.findRoute('link1', toEdge, 'connected', routingMode=traci.constants.ROUTING_MODE_AGGREGATED)
        # traci.simulation.findRoute('link2', toEdge, 'connected', routingMode=traci.constants.ROUTING_MODE_AGGREGATED)
        temp_path = stageResult.edges[1:-1]

        if temp_path[0] == 'link1':
            self.action = 1
        elif temp_path[0] == 'link2':
            self.action = 2
        ...
        if self.action != lead_action:
            ...
            if temp_path == self.lead_path:
                ...
                #print('d2_distance here', original_time)
            else:
                #print(temp_path, self.lead_path)
                i = 0
