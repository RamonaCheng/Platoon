...
class JuctionAgent:
    def __init__(self, observation_space, action_space):
        ...
        self.initial_value = [5.0, 10.0, -3.0]
    ...
    def get_state(self, junction_vehicle, junction_time):
        if self.vehicle_count == 0:
            ...
            if temp_des == 2:
                ...
                self.lead_path = traci.simulation.findRoute('link5', 'link11', 'connected').edges
            elif temp_des == 3:
                # select the shorest path
                traci.vehicle.setVia(junction_vehicle, ['link5'])
                traci.vehicle.rerouteEffort(junction_vehicle)
                ...
                # change the current vehicle as the leading vehicle
                self.lead_vehicle_link = 5
                self.lead_path = traci.simulation.findRoute('link5', 'link16', 'connected').edges
    ...
    def take_action(self, state, junction_vehicle):
        ...
        if temp_path[0] == 'link5':
            self.action = 5
        elif temp_path[0] == 'link6':
            self.action = 6
