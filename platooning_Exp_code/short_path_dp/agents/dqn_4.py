...
@Description: 
@LastEditTime: 2023-03-31 21:31:31
'''
import gym
import random
import numpy as np
...
class JuctionAgent:
    ...
    def get_state(self, junction_vehicle, junction_time):
        if self.vehicle_count == 0:
            ...
            if temp_des == 2:
                ...
                self.lead_path = traci.simulation.findRoute('link3', 'link11', 'connected').edges
            elif temp_des == 3:
                # select the shorest path
                ...
                self.lead_path = traci.simulation.findRoute('link4', 'link19', 'connected').edges
    ...
    def take_action(self, state, junction_vehicle):
        ...
        #print(fromEdge, toEdge, stageResult)

        if temp_path[0] == 'link3':
            self.action = 3
        elif temp_path[0] == 'link4':
            self.action = 4
