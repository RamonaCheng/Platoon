...
def generate_routefile():
	...
	start_0_arrival_rate = [0.015]
	start_1_arrival_rate = [0.015]
	start_2_arrival_rate = [0.025]
...
<routes>
    ...
    <vType id="connected" vClass="passenger" tau="5.5" minGap="7.5" carFollowModel="IDM" maxSpeed="40.00" emergencyDecel="4.5"/>
	...
	def collectFiles(depart_time_list,routes,fromLoc,toLoc,allTrips, HV_ratio):
		for j in range(0, len(depart_time_list)):
			...
			if random_num >= HV_ratio:
				...
				allTrips.append(Trip_Normal(depart_time_list[j],fromLoc,toLoc,j))	# the case for HVs
	...
	depart_time_list=generateDepartTime(start_0_arrival_rate,1000,False)
	fromLoc="link1"
	toLoc="link3"
	HV_ratio = 0.0
	collectFiles(depart_time_list,routes,fromLoc,toLoc,allTrips, HV_ratio)

	depart_time_list=generateDepartTime(start_1_arrival_rate,1000,False)
	fromLoc="link7"
	toLoc="link3"
	HV_ratio = 0.0
	collectFiles(depart_time_list,routes,fromLoc,toLoc,allTrips, HV_ratio)

	depart_time_list=generateDepartTime(start_2_arrival_rate,2000,True)
	fromLoc="link8"
	toLoc="link3"
	HV_ratio = 0.0
	collectFiles(depart_time_list,routes,fromLoc,toLoc,allTrips, HV_ratio)
