...
class JuctionAgent:
    ...
    def take_action(self, state):
        ...
        if sk > 40.0:
            ...
            if len(self.memory) < 40:
                ...
                cost_catch_up = self.beta_1[0] * sk_norm ** 0 + self.beta_1[1] * sk_norm ** 1 + self.beta_1[2] * sk_norm ** 2 + self.beta_1[3] * sk_norm ** 3 + self.beta_1[4] * sk_norm ** 4 + self.beta_1[5] * sk_norm ** 5
        ...
        #print('junction 2 action: ', self.action)
        return self.action

    def update_onestep(self):
        # return previous node
        current_link = traci.vehicle.getRoadID(self.temp_vehicle)
        if current_link == 'link2' and self.temp_vehicle.split('_')[1] != '0':
            previous_node = 1
        else:
            previous_node = 0
    ...
    def add_to_memory(self, current_vehicle_id, min_q, departure_link_fuel, departure_link_time):
        ...
        for item in list(self.temp_memory):
            if current_vehicle_id == item[0]:
                ...
                if item[2] == 0:
                    self.memory_0.append((item[1], item[2], min_q, vehicle_cost))
                else:
                    self.memory_1.append((item[1], item[2], min_q, vehicle_cost))
    ...
    def show_curve(self):
        ...
        for sk_test in sk_list_test:
            ...
            estimated_cost = self.beta_1[0] * sk_test ** 0 + self.beta_1[1] * sk_test ** 1 + self.beta_1[2] * sk_test ** 2 + self.beta_1[3] * sk_test ** 3 + self.beta_1[4] * sk_test ** 4 + self.beta_1[5] * sk_test ** 5
