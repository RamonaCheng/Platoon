                ...
                if len(traci.inductionloop.getLastStepVehicleIDs("e1Detector"+lane)) > 0:
                    for loop_vehicle_1 in traci.inductionloop.getLastStepVehicleIDs("e1Detector"+lane):
                        loop_vehicle_type = traci.vehicle.getTypeID(
                            loop_vehicle_1)
                        # if loop_vehicle_type in ['connected_pFollower', 'connected_pLeader', 'conventional']:
...
class network:
    ...
    def observe(self):
        ...
        for vehicle in traci.vehicle.getIDList():
            ...
            if "Platooning" in vehicle_item_type:
                if "_1" not in vehicle_lane:
                    # Ensure CAVs stay in dedicated lanes
                    traci.vehicle.changeLane(vehicle, 1, 50.0)
            else:
                if "_1" in vehicle_lane:
                    # Ensure conventional vehicles do not enter CAV lanes
                    traci.vehicle.changeLane(vehicle, 0, 50.0)

            #change colors based on vehicle type
            if (vehicle_item_type == 'connected_pFollower' or vehicle_item_type == 'connected_pCatchup' or vehicle_item_type == 'connected_pCatchupFollower'):
                ...
                traci.vehicle.setColor(vehicle, (0, 255, 255))  # cyan
